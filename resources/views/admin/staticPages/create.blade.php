...
@section('content')
<div class="p-4 bg-white rounded-lg shadow-md max-w-3xl mx-auto">
    <h2 class="text-xl font-bold text-gray-700 mb-4">Create Static Page</h2>
    ...
    <form method="POST" action="{{ route('admin.staticpages.store') }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content_summernote" class="w-full border rounded p-2 text-sm">{{ old('content') }}</textarea>
            @error('content')
                <span class="text-red-500 text-xs">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="flex space-x-2 mt-4">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-sm">Create</button>
            <a href="{{ route('admin.staticpages.index') }}" class="bg-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-400 text-sm">Cancel</a>
        </div>
    </form>
</div>
